...
public struct BuildHospitalExtraConfig
{
    ...
    //[Tooltip("Weight applied to infection levels when estimating cell score. Higher values increase the penalty for higher infection levels.")]
    //public float infectionBonusWeight;
}
...
public class BuildHospitalActionSO : HumanActionSO
{
    ...
    private void OnValidate()
    {
        ...
        if (Mathf.Abs(benefitWeightSum - 1f) > 0.0001f)
            ...
            Debug.Log($"Benefit weights of BuildHospitalActionSO are valid. Current sum = {benefitWeightSum}");
